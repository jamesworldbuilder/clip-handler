...
// caches time display element locally to prevent query spam during animation frame loop
...
function renderScrubberAnimation() {
    if (!video.paused) {
        ...
        if (timeDisplay) timeDisplay.innerText = `${formatTime(video.currentTime)} / ${formatTime(video.duration || 0)}`
...
export function initTransport() {
    ...
    // sets initial video mute state to match ui checkbox
    video.muted = muteToggle.checked

    // syncs mute state to checkbox changes
    ...
    // binds double click event to scrubber to reset video playback
    scrubber.addEventListener('dblclick', () => {
        ...
        const timeDisplay = document.getElementById('time-display')
        if (timeDisplay) {
            timeDisplay.innerText = `${formatTime(0)} / ${formatTime(video.duration || 0)}`
        }
        
        // forces immediate ui sync across application
